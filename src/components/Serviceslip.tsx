...
// //   /* ---------------- LOAD USERS + EQUIPMENTS ---------------- */
// //   useEffect(() => {
// //     if (!isAccepted) return;

// //     axiosClient.get("/users", { params: { unitId: ticket.unitId } })
// //       .then(res => setUsers(res.data.users));

// //     axiosClient.get("/equipments", { params: { unitId: ticket.Id } })
...
// //   };
// const handleAccept = async () => {
//     if (isAccepted || isAccepting) return; // 🔒 HARD GUARD
...
//     axiosClient
//       .get("/users", { params: { unitId: ticket.unitId } })
//       .then((res) => setUsers(res.data.users));
...
//   return (
//     <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 p-6 overflow-auto">
//       <div className="w-full max-w-4xl bg-white rounded-3xl shadow-xl">
//         {/* 🔽 EVERYTHING BELOW IS YOUR UI — UNCHANGED */}
...
//         {/* ASSIGN SECTION */}
//         {isAccepted && (
//           <div className="border-t px-8 py-6 bg-slate-50">
//             <h3 className="font-semibold mb-4">Assign Work</h3>
...
const ServiceSlip: React.FC<ServiceSlipProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-900/60 flex items-center justify-center overflow-auto">
      <div className="w-full h-full max-w-5xl bg-white rounded-none md:rounded-3xl shadow-xl">

        {/* HEADER */}
        {/* <div className="flex justify-between items-center px-8 py-6 border-b">
          <div>
            <p className="text-xs tracking-widest text-slate-400">SERVICE CALL SLIP</p>
        ...
        <div className="px-8 py-6 space-y-6">
          ...
          <div className="grid md:grid-cols-2 gap-4 bg-slate-50 p-4 rounded-xl">
            <div><strong>Unit:</strong> {ticket.department || FALLBACK_UNIT}</div>
          ...
          <div className="grid md:grid-cols-2 gap-6">
            <div>
              ...
              <p><strong>Requested By:</strong> {ticket.createdBy}</p>
            </div>
            <div>
              <p><strong>Status:</strong>
                <span className={`ml-2 px-3 py-1 rounded-full text-xs ${statusPill(ticket.status)}`}>
                  {ticket.status}
                </span>
              </p>
            </div>
